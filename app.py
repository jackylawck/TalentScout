...
if 'analysis_results' not in st.session_state:
    st.session_state.analysis_results = {}  # 格式: {"filename": data_dict}
if 'hr_feedback_history' not in st.session_state:
    st.session_state.hr_feedback_history = {} # 格式: {"filename": [feedback_list]}
...
def build_evaluation_prompt(lang, is_ref, urgency, special, jd, cv, feedback=""):
    ...
    return f"""
You are an Elite HR Tech System executing advanced Talent Science algorithms. Evaluate this SINGLE candidate against the JD using Competency Modeling, ATS Keyword Extraction, DEI Safeguards, and Structured Interview Rubrics.
...
{{
  "funnel_and_ats": {{
    ...
    "time_to_fill_assessment": "Assessment of notice period based on urgency."
  }},
  "competency_breakdown": [
    {{"dimension": "Hard Skills & Domain", "score": "80/100", "justification": "...", "evidence": "Quote from CV"}}
  ],
  "dei_and_risks": {{
    "dei_safeguard_applied": "Explicitly identify any biased keywords (e.g., age, gender, origin) in JD or CV, and state how bias was mitigated.",
...
def process_single_candidate(cand_name, cv_content, hr_feedback=""):
    ...
    with st.status(get_ui("status_analyzing").format(cand_name), expanded=True) as status:
        try:
            ...
            status.update(label=f"✅ {cand_name} 分析完成", state="complete", expanded=False)
            return parsed_data
        except Exception as e:
...
if st.session_state.analysis_results:
    ...
    tabs = st.tabs([f"👤 {name}" for name in cand_names])
            ...
            with col_eval:
                if st.button(get_ui("re_eval_btn"), key=f"btn_{cand_name}", use_container_width=True):
                    if new_fb.strip():
                        ...
                        # Find the actual cv text from the uploaded files
                        target_cv_text = ""
                        for f in cv_files:
                            if f.name == cand_name:
